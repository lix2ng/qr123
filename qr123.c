 ...
 *    1: Os level with iterative GF MUL.
 *    2: O2 level with unrolled assembler GF MUL (Thumb-2 only).
 *    3: O3 level with log/exp LUT-based GF MUL.
 */
#define QR_SPEED 2
...
bool qr_eval(qr_ctx *ctx, uint ver, const uint8_t *data, uint len)
{
   static const uint8_t _params_blob[] = {                                 //
      ...
      44, 10, 0xd8, 0xc2, 0x9f, 0x6f, 0xc7, 0x5e, 0x5f, 0x71, 0x9d, 0xc1,  // V2
      70, 15, 0x1d, 0xc4, 0x6f, 0xa3, 0x70, 0x4a, 0x0a, 0x69, 0x69, 0x8b,  // V3
   ...
   switch (ver) {
   case 1: break;
   case 2: params += 9; break;
   case 3: params += 21; break;
...
#define _OPT __attribute__((optimize("O3")))
...
#elif QR_SPEED == 2 && defined(__thumb2__)

#define _OPT __attribute__((optimize("O2")))

static inline uint _rs_mul(uint x, uint y)
{
   // Unrolled Thumb-2 code; M4 has a 12kc saving due to free IT (15*825).
   uint prim, z;
   // clang-format off
   asm(
      "mov        %[prim], #0x11d" br
      ...
      :[x]"+r"(x), [y]"+l"(y), [z]"=&l"(z), [prim]"=&l"(prim) : :"cc"
   );
   // clang-format on
   return z;
}
...
#define _OPT __attribute__((optimize("Os")))
 ...
 */
static inline _inl bool _is_data(uint x, uint y, uint size_m1)
{
   if (x == 6 || y == 6) return false;
   if (y <= 8) return x >= 9 && x <= size_m1 - 8;
   if (y >= size_m1 - 7 && x <= 8) return false;
   if (size_m1 > 20 && x >= size_m1 - 8 && x <= size_m1 - 4 && y >= size_m1 - 8
      && y <= size_m1 - 4)
      return false;
   return true;
}
 ...
 */
static uint64_t _zigzag_step(uint x, uint y, uint size_m1)
{
   poly64_t r;
   while (true) {
      switch ((x - (x > 6)) & 3) {
 ...
 */
static void _place_data(qr_ctx *ctx, uint8_t *buf)
{
   poly64_t xy;
   ...
   if (size_m1 > 20) nbits += 7;

   for (int i = 0; i < nbits; i++) {
      bool mask0 = (xy.u0 + xy.u1) % 2 == 0;
      bool dot = buf[i / 8] & (0x80u >> i % 8);
      if (dot ^ mask0) ctx->bmp[xy.u1] |= 0x80000000u >> xy.u0;
      xy.bits = _zigzag_step(xy.u0, xy.u1, size_m1);
   }
}

void _OPT qr_encode(qr_ctx *ctx)
{
   if (!ctx) return;
...
int main(int argc, const char *argv[])
{
   ...
   if (argc < 3) {
      printf("Give a version number and a string.\n");
      return -1;
   }
   bool ok =
      qr_eval(ctx, atoi(argv[1]), (const uint8_t *)argv[2], strlen(argv[2]));
   if (!ok) {
